				...
				<div class="tab-content" id="myTabContent">
					<div class="tab-pane fade show active " id="booking"
						...
						<table class="table table-dark text-center border-warning">
                            ...
                            <tbody>
                            <c:forEach items="${dshome}" var="item">
                            <c:if test="${item.banner !=null}">
                                <tr>
                                    <th scope="row">${item.idhome }</th>
                                    <td>${item.link }</td>
                                    <td>${item.banner }</td>
                                    <td><a href="/finaldam/Banner/edit?	index=${idadmin }&&id=${item.idhome }" class="btn btn-warning text-center rounded-pill"> sửa</a></td>
                                    <td><a onclick="confirmdelete(${idadmin },${item.idhome })" class="btn btn-danger text-center rounded-pill"> xóa</a></td>
                                </tr>
                            </c:if>
                                </c:forEach>
                            </tbody>
					...
					<div class="tab-pane fade" id="profile" role="tabpanel"
						aria-labelledby="food-tab">
							<a type="submit" href="/finaldam/Poster/insert?index=${idadmin }" class="btn btn-success text-center mt-3">Thêm</a>
                            ...
                            <tbody>
                                ...
                                <c:if test="${item.poster !=null}">
                                    <tr>
                                        ...
                                        <th>${item.link }f</th>
                                        <th>${item.title }</th>
                                        <th>${item.summaryContent }</th>
                                        <th><a href="/finaldam/Poster/edit?index=${idadmin }&&id=${item.idhome }"
                                                class="btn btn-success text-center rounded-pill">sửa</a></th>
                                        <th><a onclick="confirmdelete(${idadmin },${item.idhome })"
                                                class="btn btn-danger text-center rounded-pill">xóa</a></th>
                                    </tr>
                                </c:if>
                                </c:forEach>
                            </tbody>
    ...
    <script type="text/javascript">
		...
		function confirmdelete(index,id) {
			...
			if(ques){
				...
				window.location.href='/finaldam/DeleteHome?index='+index+'&&id='+id;
			}
		}
	</script>
</body>
