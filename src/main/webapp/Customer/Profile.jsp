...
<body>
  ...
  <path d="M7.5 3a.5.5 0 0 1 .5.5v5.21l3.248 1.856a.5.5 0 0 1-.496.868l-3.5-2A.5.5 0 0 1 7 9V3.5a.5.5 0 0 1 .5-.5z"/>
		...
		<div class="row mt-8 offset-2 col-8 bg-light mt-8"
			...
			<form class="row g-3" action="/finaldam/Profile" method="POST">
				...
				<div class="form-groub col-md-4">
					<label for="username" class="bg-light">fullname</label> <input
						...
						id="fullname" required value="${user.fullname }">
				</div>
				<div class="form-groub col-md-6">
					<label for="email" class="form-label">Email</label> <input
						type="email" class="form-control border-warning" name="email"
						value="${user.email }" aria-describedby="emailid" required>
						<%
								String err = request.getParameter("error");
								if ("1".equals(err)) {
									out.print("<small id='emailhid' style='color: red;' class='form-text text-muted' >Email invalidate</small>");
								}
			%>
				</div>
				<div class="form-groub col-md-4">
					<label for="phone" class="bg-light">Number Phone</label> <input
						type="number" class="form-control border-warning" name="sdt"
						id="sdt" value="${user.sdt.trim() }" aria-describedby="phonehid">
						<%
								String err1 = request.getParameter("error");
								if ("2".equals(err)) {
									out.print("<small id='sdthid' style='color: red;' class='form-text text-muted' >Number phone invalidate</small>");
								}
						%>
				</div>
				...
				<div class="col-12 offset-6">
					<button class="btn btn-success rounded-pill border-danger">Change</button>
				</div>
			</form>
		</div>
	</main>
